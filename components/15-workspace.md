...
- The cross-session **AppArmor re-confine** on a reused slot is the one open sub-claim. **Confirmed (kb-orchestrator production config):** every stage is `apparmor: true`, and each stage is a **fresh `create_session` → new session-id → its own `jaato-ws-{sid}` profile *name***, while the slot reuses the prior stage's **runner process + `RunnerRPCClient`**. The enabling kernel rule is shipped (`change_profile -> jaato-ws-*`, template v28) and a *fresh* confine runs `aa_change_profile` in bootstrap step 1c. **Still open:** whether the *reused* runner process actually calls `aa_change_profile` from `jaato-ws-{S(N)}` → `jaato-ws-{S(N+1)}` per session, or keeps running under the prior stage's profile — the reuse path reuses the RPC client + `reset_for_slot_reuse` without a source-visible re-confine call, and the cascade side can't observe the server-side mechanism (historically touchy, the PR#94-era `change_profile from <X> fails`). *Being settled empirically: the kb-orchestrator is capturing per-stage runner PID + `/proc/<pid>/attr/current` on its next run (same PID across stages ⇒ slot reuse; the profile name per stage ⇒ whether it re-confines). This doc will record the verdict once that data lands.*
