...
- **Reactor-spawned stage sessions attach under `_HEADLESS_CLIENT_ID`** (`= "_headless"`, `session_manager.py:4764`). Their events still reach the cascade owner because `_emit_to_session` fans out *additionally* to the cascade-client registry by cid (`_dispatch_to_cascade_clients`, `session_manager.py:3403`).
...
- `jaato/jaato-sdk/jaato_sdk/events.py` — `AgentCompletedEvent` (class :392, typed `payload` :410) + `SlotSettledEvent` (`was_warm` :549, class :518) for the production warm-slot handoff. The validated payload is **hoisted onto the bus event** the reactor receives by `jaato/jaato-server/server/core.py` `_server_event_to_bus_event` (events.py only *declares* the field).
- `jaato/jaato-server/server/session_manager.py:4764` / `:3329` — `_HEADLESS_CLIENT_ID` stage identity + `_dispatch_to_cascade_clients` event bridging to the `_cascade:{cid}` owner.
- `jaato/jaato-server/server/runner_pool.py:284` — `acquire_slot(cascade_driver_id=...)` best-effort warm affinity.
- `jaato/jaato-sdk/jaato_sdk/events.py:410` (`AgentCompletedEvent.payload`) — typed payload is attached only when the producer profile declares a `completion_payload_schema`; otherwise `None` (legacy summary).
