...
- **(a) Per-step fan-out → fan-in.** A planning stage can write a list of work items; a reactor spawns one session per item (fan-out). The fan-in is **composed, not a built-in barrier** — there is no join primitive in the engine: a handler counts completed items against the remaining work in cascade state and spawns the join stage only when none remain, so the join runs exactly once after all the fanned-out steps complete.
...
- `jaato/jaato-sdk/jaato_sdk/events.py` — `AgentCompletedEvent` (typed `payload`) + `SlotSettledEvent` (`was_warm`) for the production warm-slot handoff. The validated payload is **hoisted onto the bus event** the reactor receives by `jaato/jaato-server/server/core.py` `_server_event_to_bus_event` (events.py only *declares* the field).
- `jaato/jaato-server/server/session_manager.py` — `_HEADLESS_CLIENT_ID` stage identity + `_dispatch_to_cascade_clients` event bridging to the `_cascade:{cid}` owner.
- `jaato/jaato-server/server/runner_pool.py` — `acquire_slot(cascade_driver_id=...)` best-effort warm affinity.
- `jaato/jaato-sdk/jaato_sdk/events.py` (`AgentCompletedEvent.payload`) — typed payload is attached only when the producer profile declares a `completion_payload_schema`; otherwise `None` (legacy summary).
