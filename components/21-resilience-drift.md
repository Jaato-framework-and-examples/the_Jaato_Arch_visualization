...
A **premium-shipped reactor**, distributed as an **opt-in example** (`jaato_premium/reactors/examples/drift_monitor/` — a `reactors/drift_monitor.json` rule fragment + a thin `scripts/drift_monitor.py` shim). It is **not** auto-installed — premium writes nothing to `~/.jaato/`. A deployment opts a *workspace* in by copying the example into that workspace's `.jaato/` (`cp -r …/examples/drift_monitor/. <ws>/.jaato/`), and the reactor engine's **workspace-tier discovery** picks it up for that workspace only (copy into `~/.jaato/` instead to run it globally — a deliberate opt-in). The reactor *framework* loads via the `jaato.extensions` → `reactors` entry point (`pyproject.toml`). All real logic lives in the importable `jaato_premium.drift_monitor.reactor_logic.handle_event` (`reactor_logic.py`), kept alive in the import cache across the framework's per-dispatch script reload so per-session `DriftState` survives between turns — the shim only routes events to it.

It scores from three event types (`reactor_logic.py` docstring): `agent.output` chunks **accumulate** the model's streaming reasoning into a per-session buffer; `plan.step_updated` is the **primary** scoring trigger — it scores the buffer against the **current** step's goal *before* applying the transition (agents routinely open and close several steps within one turn, so per-step scoring attributes reasoning to the right step); `turn.completed` is the fallback for multi-turn steps. Scoring (`_flush_and_score`, `reactor_logic.py`): embed the buffer (sentence-transformers `all-MiniLM-L6-v2`, `embeddings.py`), `cosine_similarity` to the step-goal embedding (`scoring.py`, `state.py`).

`DriftState.is_drifting` (`state.py`) flags on **three conditions**: (1) **adaptive** — score below `mean − DEVIATION_FACTOR·std` of the first `BASELINE_TURNS` turns; (2) **static** — score `< HARD_THRESHOLD = 0.30`; (3) **trajectory** — the last `TRAJECTORY_WINDOW = 3` turn-scores monotonically declining (`scoring.py`). Thresholds are framework constants, not configurable in v1 (`state.py`). On **every** score it emits a `drift.measured` event (step id, score, flagged, flag type) so telemetry sees the signal even when not flagged (`_emit_drift_measured`, `reactor_logic.py`); on a flag it injects a conversational nudge via `ctx.inject_prompt` — *"…your last turn scored low against the active plan step… if this is an intentional pivot, mark the step complete or update the plan; otherwise return to the step's goal"* (`_nudge`, `reactor_logic.py`, `_DRIFT_NUDGE`). `complete_step` also runs a **strategic-drift** check across completed steps against the overall plan goal (`state.py`).

### 2. Reliability — behavioral patterns & tool-trust (in-process plugin → event-driven reactor, shipped)
The reliability mechanism is the *behavioral* counterpart to the drift monitor, and its **logic is public** (`shared/plugins/reliability/`): an **adaptive tool-trust ledger** — every tool result keys a `FailureKey` (`types.py`) and an `EscalationRule` (`types.py`) transitions a tool `TRUSTED→ESCALATED→RECOVERING` (`TrustState`, `types.py`) — and a **`PatternDetector`** (`patterns.py`) for repetitive calls, introspection loops, read-only stalls, announce-no-action, error-retry loops, and prerequisite-policy violations (e.g. `setStepStatus` before `createPlan`).

**The wiring was migrated to a reactor.** The original in-process `ReliabilityPlugin` (`plugin.py`) was **dead in the shipped daemon** — it needed *three* bespoke wirings no production path called (plugin attach via `configure_plugins(reliability_plugin=…)` `jaato_runtime.py`; nudge sinks via `set_nudge_callbacks`; forced-approval via `wrap_permission_plugin`), so its nudges silently no-op'd and even its `reliability …` commands were never registered. The fix mirrors the drift monitor: the *wiring* (not the logic) now lives in a **premium-gated event-driven reactor** — `jaato-premium/jaato_premium/reliability/reactor_logic.py`, shipped as an **opt-in example** under `jaato_premium/reactors/examples/reliability/` (premium reactors are opt-in examples a deployment tier-discovers per workspace, not globally auto-installed — premium #47). It subscribes to bus events — `tool.call_started`, `tool.call_completed`, `agent.output`, `turn.completed`, `plan.step_updated` — reuses the public `PatternDetector`/types, steers via a **non-blocking `ctx.inject_prompt` nudge** (`reactor_logic.py`), and emits `reliability.escalated` / `reliability.pattern_detected` (the bus `EventType` enum, `event_bus.py`) for observability — collapsing the three dead wirings into one entry point and replacing the dead nudge callbacks with the wired `inject_prompt`.

**Enforcement is presentation-aware — and shipped.** The reactor writes the escalated tool into session-attached state (`reliability:escalated_tools`); the permission plugin's read-hook `_reliability_escalation_action` (`jaato/jaato-server/shared/plugins/permission/plugin.py`, invoked per call) branches on `PresentationContext.client_type`: **interactive** sessions (`TERMINAL`/`WEB`/`CHAT`) get a re-confirm (`"ask"`), **headless** sessions (`API`) get a hard **`"deny"`** — never a blocking prompt (the cardinal invariant: a synchronous prompt would hang a cascade). It is a strict no-op until the reactor escalates, and it is unit-tested (`permission/tests/test_reliability_escalation_gate.py`). The headless deny needn't be a dead end: the reactor's `_park_for_approval` (`reactor_logic.py`) acquires a **`HandoffGate`** keyed `reliability:{session}:{tool}` (with a **TTL watchdog** so the cascade never hangs) and posts an **out-of-band notify**; a human approves elsewhere (a webhook bridge), `gate.released` fires, and a deployment **revive** reactor writes `reliability:approved_tools` — which **overrides** `escalated` in the permission hook (jaato #374), flipping deny→allow — then resumes the parked session **by id** and retries the tool. The **`reliability-exercise`** harness validated this whole loop end-to-end, across three resume strategies: the parked session kept loaded **in-place**, **unloaded** then `resume_session`d, or **polled**.

> **Implementation status — shipped & validated.** The reliability reactor is shipped in premium (`jaato_premium/reliability/reactor_logic.py` + the opt-in example `jaato_premium/reactors/examples/reliability/`; premium #47). The **presentation-aware enforcement** (`permission/plugin.py` — interactive→`ask`, headless→`deny`, `approved`-overrides-`escalated`) is **merged and unit-tested centrally** in jaato, and the whole loop — detect → escalate → headless-deny + nudge, then the out-of-band-approval **deny→allow flip** across three resume tiers — is **validated end-to-end** by the `reliability-exercise` harness. Substrate: `reliability.*` bus events (`event_bus.py`) + `is_error_result` on `tool.call_completed` (`events.py`, so a "success-but-error-body" still counts as a failure). Framework support landed centrally (jaato #374/#379/#391/#393/#394, premium #43/#44/#45/#47 — premium #43 fixed a real "steer+enforce was silently dead" bug the harness was built to surface). The trust ledger is **per-session** (reactor-native). Design source of truth: `jaato/docs/design/reliability-event-driven-migration.md`.

### 3. Memory continuity scopes — intent that survives across sessions
Each memory carries a `scope` ∈ `{SCOPE_PROJECT, SCOPE_UNIVERSAL}` (`memory/models.py`). Project-scope memories live in the **workspace** store (`.jaato/memories.jsonl`); universal-scope ones route to a fixed **global** store `~/.jaato/memories.jsonl` shared by all agents and sessions (`plugin.py`). Each is stamped with `source_agent`/`source_session` (`plugin.py`). This is the cross-session intent anchor: an agent tags a consolidated lesson with a stable scope, and a later session — even in another workspace, for universal scope — re-surfaces it automatically, so the agent "remembers prior runs" without an explicit lookup (`docs/design/agent-continuity.md`).
...
## Source references
- `jaato-premium/jaato_premium/drift_monitor/reactor_logic.py` — `handle_event` (event routing); `_flush_and_score` (embed + score + detect + nudge); `_emit_drift_measured`; `_nudge` / `_DRIFT_NUDGE`.
- `jaato-premium/jaato_premium/drift_monitor/state.py` — `DriftState.is_drifting` (adaptive / static / trajectory); thresholds; `score_text`; `complete_step`/`_check_strategic_drift`.
- `jaato-premium/jaato_premium/drift_monitor/scoring.py` — `cosine_similarity`; `compute_adaptive_baseline`; `detect_trajectory_decline`.
- `jaato-premium/jaato_premium/drift_monitor/embeddings.py` — `MODEL_NAME = "all-MiniLM-L6-v2"`; `SentenceTransformerProvider`. Shipped as an **opt-in example** at `jaato_premium/reactors/examples/drift_monitor/` (not auto-installed — premium `pyproject.toml`); the reactor framework loads via `jaato.extensions → reactors` (`pyproject.toml`).
- `jaato/jaato-server/shared/plugins/reliability/` — the public reliability **logic** the reactor reuses: `FailureKey`/`EscalationRule`/`TrustState` (`types.py`), `PatternDetector` (`patterns.py`). The in-process `ReliabilityPlugin` (`plugin.py`) is **dead** — its three activation seams (`configure_plugins(reliability_plugin=…)` `jaato_runtime.py`; `set_nudge_callbacks`; `wrap_permission_plugin` `plugin.py`) are never called outside tests (daemon `core.py` passes 3 args).
- `jaato/jaato-sdk/jaato_sdk/event_bus.py` — `RELIABILITY_ESCALATED`/`RELIABILITY_PATTERN_DETECTED` bus event types (substrate, #318); `is_error_result` on `tool.call_completed` (`jaato-sdk/jaato_sdk/events.py`, #319).
- `jaato-premium/jaato_premium/reliability/reactor_logic.py` — the shipped reliability reactor: `_nudge` (`inject_prompt`), `_on_escalation` (emit + write `reliability:escalated_tools`), `_park_for_approval` (`HandoffGate` for headless out-of-band approval); opt-in example at `jaato_premium/reactors/examples/reliability/` (premium #47).
- `jaato/jaato-server/shared/plugins/permission/plugin.py` — `_reliability_escalation_action` (presentation-aware enforcement: interactive→`ask`, headless→`deny`, `approved`-overrides-`escalated`, jaato #374); unit-tested at `permission/tests/test_reliability_escalation_gate.py`. End-to-end harness: the `reliability-exercise` repo (Phase-1 deny + Phase-2 deny→allow flip across three resume tiers). Design: `jaato/docs/design/reliability-event-driven-migration.md`.
- `jaato/jaato-server/shared/plugins/memory/models.py` — `SCOPE_PROJECT`/`SCOPE_UNIVERSAL`; scope→store routing `plugin.py`; global store `~/.jaato/memories.jsonl` `plugin.py`; source stamping `plugin.py`. (Stores are JSONL files, remapped to a `.jaato/memories/` dir for the raw/curated split — see doc 20.)
- `jaato/jaato-server/shared/plugins/memory/indexer.py` — `find_matches_in_text` (tag-coherence, no embeddings); memory enrichment priority 80 (`plugin.py`).
- `jaato/jaato-server/shared/plugins/references/plugin.py` — hybrid `_enrich_content` (tag pass + semantic veto 0.4 + semantic add 0.75); references priority 20; embedding types/discovery `references/embedding_types.py`.
