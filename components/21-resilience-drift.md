...
All four converge on the same lever: **write intent back into the model's context** (a nudge, a re-injected reference, a resurfaced memory) and **emit a signal** (a `drift.measured` event, a `reliability.*` telemetry event) so the drift is observable.
