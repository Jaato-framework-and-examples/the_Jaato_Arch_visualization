...
Pydantic was chosen over plain dataclasses so the same definitions drive JSON Schema export for TypeScript SDK codegen, with a frozen wire format (`events.py:1-19`). The wire protocol carries a semver `PROTOCOL_VERSION = "1.0"` (`events.py:55`); `Event.model_config` sets `extra='ignore'` so an older client silently drops unknown fields from a newer server (`events.py:296`) — forward compatibility by construction.
...
`EventType(str, Enum)` (`events.py:62`) is the discriminator — string values like `"agent.output"`, `"tool.call_start"`, `"slot.settled"`. Every event is an `Event` subclass (`events.py:287`) that pins its `type` field to one enum member. Serialization is `event.to_dict()` / `to_json()` via `model_dump(mode='json')` so str-enums emit as their string values. The same module holds both **server→client** events and **client→server** requests (the `*Request` classes near `events.py:1241+`), plus value objects shared across events (`UsageBreakdown` at `events.py:816` carried identically by `TurnProgressEvent`, `TurnCompletedEvent`, and `ContextUpdatedEvent`).

### Snapshot-on-connect

`SessionInfoEvent` (`events.py:1034`) is the "full state snapshot" pushed on connect/attach: current session id/name, model provider/model, plus lists the client needs for display and completion (`sessions`, `tools`, `models`, `user_inputs`, `memories`, `sandbox_paths`, `services`, `tool_id_mappings`). This is why reconnection needs no event-replay log — the daemon's `emit_current_state()` re-sends the snapshot (`core.py:1346`, `core.py:1371`) followed by an `AgentCreatedEvent` per tracked agent.
...
- **Client→server requests** — `SendMessageRequest` (`events.py:1241`), `PermissionResponseRequest`, `ClarificationResponseRequest`, `StopRequest`, `CommandRequest`, plus typed SDK-parity verbs (`InjectPromptRequest`, `ReplayMessagesRequest`, permission-policy verbs).
...
3. **A turn begins.** Client sends `SendMessageRequest` (client→server). Daemon emits `AgentStatusChangedEvent(status="active")` (`core.py:2901`).
...
- `AgentCompletedEvent` (`events.py:385`) fires *early* — when the agent signals completion, while its runner is still up. A reactor handling it only **persists the next-stage spawn spec**; it does not spawn yet, because spawning now would race the slot teardown and **cold-spawn** (~30s).
...
## Source references
- `jaato/jaato-sdk/jaato_sdk/events.py:62` — `EventType(str, Enum)`, the wire discriminator (`"agent.output"`, `"slot.settled"`, …).
...
- `jaato/jaato-server/server/core.py:2901,2920,2941` — emit sites for `AgentStatusChangedEvent`, `AgentCompletedEvent`, `SessionTerminatedEvent`.
- `jaato/jaato-server/server/core.py:5428-5450` — `SlotSettledEvent` emitted at end of `JaatoServer.shutdown`, gated on `cascade_driver_id`, after warm slot returns.
