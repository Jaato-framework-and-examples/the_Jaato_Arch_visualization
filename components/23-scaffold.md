...
- `jaato/jaato-server/shared/scaffold/_client_templates.py` — known-good client recipes (`ClientType.API` keeps `signal_completion`; cold-autostart timeout; cascade `cascade_driver_id`); `--recoverable` → `IPCRecoveryClient`.
