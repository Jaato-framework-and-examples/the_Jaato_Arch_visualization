...
**The deeper link — pausing a *cascade* for out-of-band approval.** `onPermission` assumes a client is connected to answer. But a jaato permission escalation is a **bus event**, so a **reactor** can handle it with *nothing connected*. The reliability pattern (resilience doc): a **headless cascade stage** (Example 9) keeps failing a tool → the reliability reactor escalates → a reactor **parks the call on a `HandoffGate`** and requests approval **out-of-band** — e.g. a Telegram bot carrying the tool, its args, and which cascade stage asked → on **approve**, a second reactor flips deny→allow and **drives that same session's retry by id**, even if its runner was unloaded while waiting. So a long-running **cascade (Example 9) can pause mid-flight for a human and resume on approval** (within the gate's TTL, and the gate registry survives a daemon restart) — no client attached, no polling. Mastra's workflow `suspend`/`resume` does the same *shape*, but resumption runs in **your** process: you must be alive (and holding the run) to call `run.resume()`. jaato's pause → approve → resume is **daemon-side, out-of-band, and durable**. (It's a deployment pattern — premium reactors + a gate + an approval channel you wire — not client SDK code; mechanism in the resilience doc.)
