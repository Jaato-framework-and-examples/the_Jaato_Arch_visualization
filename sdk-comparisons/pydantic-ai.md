...
- **jaato-sdk** is a **runtime you run two ways**: `jaato.session(mode=…)` runs the *same* agent **embedded in your process** (no daemon — like Pydantic AI) **or** against a **long-lived daemon** (local `ipc` or remote `ws`), where each agent is an isolated, permission-gated, per-session subprocess. Either way your code opens a **session** and `ask`s — the agent loop, tools, persistence, and permissions live in the **runtime**; the daemon adds isolation, multi-tenancy, and recovery.

So the trade is: Pydantic AI gives you a tightly-typed agent that lives in your process and is trivial to test and embed; jaato gives you a runtime/provider-agnostic agent you can run **the same way — in your process** — *and* move **behind a daemon boundary** for multi-tenancy, isolation, and recovery when you want it, with typed *completion* gates enforced by the runtime. Read it as a trade, not a scoreboard.
